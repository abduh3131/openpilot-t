...
from autonomy.utils.data_structures import DetectedObject, NavigationDecision
...
class Navigator:
    """Generates high level navigation goals using obstacle detections."""

    def __init__(self, config: NavigatorConfig | None = None) -> None:
        self.config = config or NavigatorConfig()
        self._last_bias = 0.0

    def plan(self, detections: Iterable[DetectedObject], frame_size: tuple[int, int]) -> NavigationDecision:
