...
The GUI offers start/stop controls for `launch_openpilot.sh`, shows which camera sensor is active, surfaces device thermals, and displays actuator values in real time.
...
4. Starts a `tmux` session named `openpilot` with two windows:
   * **Window 0:** Runs `./launch_openpilot.sh`, which starts the manager and the full openpilot process stack.
...
By default the monitor subscribes to the `carControl` message stream (controller commands before safety limits). To see the post-safety output, pass `--topic carOutput`. Adjust the print frequency with `--rate` (in Hz) if the output scrolls too quickly.
...
* **No actuator messages:** openpilot must be running and publishing `carControl`/`carOutput`. In replay scenarios ensure the log is streaming, or use `--topic carOutput` to tap the output of the car interface.
